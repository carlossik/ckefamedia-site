              ...
              <div className="workflow-actions workflow-actions--card">
                ...
                {booking.operations_status === 'awaiting_confirmation' ? <>
                  <div className="workflow-fields">
                    <label className="field"><span>Agreed price before discount (£)</span><input type="number" min="0.01" step="0.01" value={quoteDrafts[booking.id] ?? (booking.quoted_total_pence !== null ? (booking.quoted_total_pence / 100).toFixed(2) : '')} onChange={(e) => setQuoteDrafts((current) => ({ ...current, [booking.id]: e.target.value }))} placeholder="e.g. 250.00" />{booking.discount_code ? <small>Code {booking.discount_code} will be applied to the final total automatically.</small> : null}</label>
